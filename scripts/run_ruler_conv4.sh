...
# Usage: bash scripts/run_ruler_conv4.sh [xattn|conv|minference|flex|full] [extra RULER args]
METHOD="${1:-conv}"
...
export RULER_NUM_SAMPLES="${RULER_NUM_SAMPLES:-1000}"
...
exec bash ./run64_1.sh llama3.1-8b-chat synthetic \
  --stride "${STRIDE:-8}" \
  --metric "${METHOD}" \
