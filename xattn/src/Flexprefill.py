...
def _topk_ratio_block_idx(
    ...
    """
    Select a fixed ratio of causal-visible key blocks for every query block.

    For query block i:

        keep_i = ceil(topk_ratio * (i + 1))

    because a causal self-attention query block i can see key blocks [0, i].

    The returned tensor contains flattened block-grid indices and has shape:

        [batch_size, num_q_heads, total_selected_blocks]

    total_selected_blocks is identical across batch/head because the per-row
    budget depends only on the query-block index.
    """
    ...
    _, k_len, num_kv_heads, k_head_dim = k.shape

    if seq_len != k_len:
        raise ValueError(
            "topk_ratio prefill currently requires q_len == k_len, "
            f"got q_len={seq_len}, k_len={k_len}"
        )
    if head_dim != k_head_dim:
        raise ValueError(
            f"q/k head_dim mismatch: {head_dim} vs {k_head_dim}"
        )
    ...
    # [B, num_blocks, Hq/Hkv, D].
    # triton_bnhd_pool divides the final partial block by its real token count.
    avg_q = triton_bnhd_pool(q, block_size, pool_type="avg")
    avg_k = triton_bnhd_pool(k, block_size, pool_type="avg")

    # Expand KV heads to query heads using the same GQA mapping as the kernels.
    if not gqa_interleave:
        avg_k = avg_k.repeat_interleave(gqa_groups, dim=2)
    else:
        avg_k = avg_k.repeat(1, 1, gqa_groups, 1)

    # Block-level importance scores: [B, Hq, Q_blocks, K_blocks].
    block_scores = torch.einsum(
        "bihd,bjhd->bhij",
        avg_q.float() / math.sqrt(head_dim),
        avg_k.float(),
    )
    ...
    q_idx = torch.arange(num_blocks, device=q.device)
    k_idx = torch.arange(num_blocks, device=q.device)

    causal_block = k_idx[None, :] <= q_idx[:, None]
    block_scores = block_scores.masked_fill(
        ~causal_block[None, None, :, :],
        -torch.inf,
    )

    # Each query block keeps the requested ratio of its own visible support.
    visible_counts = q_idx + 1
    k_per_q = torch.ceil(
        visible_counts.to(torch.float32) * ratio
    ).to(torch.long)
    k_per_q = k_per_q.clamp(min=1, max=num_blocks)

    # torch.topk accepts one scalar K. Select the largest row budget first,
    # then remove ranks beyond each query row's individual budget.
    max_topk = max(
        1,
        min(num_blocks, int(math.ceil(ratio * num_blocks))),
    )
    topk_idx = torch.topk(
        block_scores,
        k=max_topk,
    ...
    # Convert (query_block, key_block) to flattened block-grid indices.
    flat_idx = (
        q_idx[None, None, :, None] * num_blocks
        + topk_idx
    )

    block_idx = flat_idx.masked_select(keep_rank).view(
        batch_size,
        num_q_heads,
        -1,
    )

    return block_idx.contiguous()
...
def get_active_blocks(
    ...
    # Selection priority:
    #   topk_ratio > gamma/tau adaptive vertical-slash selection.
    #
    # The ratio path mirrors Conv's semantics: query block i keeps
    # ceil(topk_ratio * visible_key_blocks_i) blocks.
    if topk_ratio is not None:
        return _topk_ratio_block_idx(
            q=q,
            k=k,
...
def Flexprefill_prefill(
    ...
    # Keep the original dense short-sequence fallback only for the original
    # gamma/tau path. With topk_ratio enabled, using dense attention here would
    # violate the requested block ratio.
    if (
        topk_ratio is None
        and seq_len
        <= max(
