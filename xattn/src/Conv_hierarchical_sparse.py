...
from xattn.src.kernels_conv_hierarchical_sparse import (
...
def _topm_candidate_mask_from_proxy(
    ...
    """
    Per query block/head Top-M candidate preselection from a cheap proxy map.

    proxy_scores: [B, H, Q_blocks, K_blocks]
    """
    ...
    # Always keep a valid diagonal-like fallback so every q block has support.
    fallback_k = (
        torch.arange(qb, device=device) + int(offset)
    ).clamp(0, kb - 1)
...
def _dilate_candidate_mask_2d(
    ...
    """
    2D neighborhood dilation on the [Q_blocks, K_blocks] candidate map.

    A 7x7 kernel expands each selected candidate to its +/-3 neighborhood in
    both query-block and key-block directions. This matches the receptive field
    of the later learned 7x7 convolution.
    """
...
def conv_estimate(
    ...
    # ── Step 3: concatenate block mass and apply learned 2D Conv ──
