...
def _topk_ratio_mask_from_scores(
    ...
    """
    Select a fixed *ratio* of the valid key blocks for every query block.

    For causal prefill, query row i keeps:
    ...
    Therefore topk_ratio=0.6 means each query block keeps approximately 60%
    of its own causal-visible key blocks, rather than 60% of the globally
    padded key-block dimension.
    ...
    if causal:
        ...
        # Maximum visible support in this query chunk. This is computed with
        # Python integers, avoiding a CUDA -> CPU synchronization.
        max_visible = max(
            0,
    ...
    # Per-query variable K. ceil() guarantees at least the requested ratio;
    # rows with one visible block still keep one block.
    k_per_q = torch.ceil(
    ...
    # torch.topk needs one scalar K, so select max_topk first and then keep
    # only the first k_per_q ranks for each query row.
    idx = torch.topk(
        energy,
        k=max_topk,
        dim=-1,
    ...
    # Safety fallback for any structurally empty row.
    fallback_k = (
        q_idx + int(offset)
    ).clamp(0, kb - 1)
...
def conv_estimate(
    ...
    if topk_ratio is not None:
        ...
        if fixed_topk is not None:
            raise ValueError(
                "Use only one block budget: topk_ratio or fixed_topk, not both."
            )
    ...
    # Full inverse anti-diagonal estimator: use all offsets
    # iter = 0, 1, ..., stride - 1.  This disables the spaced sampling
    # acceleration while keeping the rest of the Conv pipeline unchanged.
    ...
    if (
        ...
        try:
            ...
            print(
                f"[Conv.py WARN] block selection failed. "
                f"topk_ratio={topk_ratio}, fixed_topk={fixed_topk}, "
                f"conv_safe_topk={conv_safe_topk}, error={repr(e)}"
            )
            raise
...
def Conv_prefill(
    ...
    When topk_ratio is set, threshold/Top-P selection is disabled.
    For causal prefill, every query block keeps:

        ceil(topk_ratio * number_of_causal_visible_key_blocks)
