...
def _get_conv_weight(kernel_size=7, weight_path=None, device=None):
    ...
    #     print("loading weight_path:",weight_path)
    #     weight = torch.load(weight_path, map_location=device, weights_only=True)
    if weight_path is None and os.path.exists(_DEFAULT_WEIGHT_PATH):
...
def conv_estimate(
    ...
    if conv_safe_topk and fixed_topk is None:
        # Explicit debug/safety mode only. Do not use this for fair score comparison
        # against xattn, because top-k selection changes sparsity behavior.
        ...
        try:
            for chunk_idx in range(q_chunk_num):
                ...
                score_chunk = attn_sums_smoothed[:, :, chunk_start:chunk_end, :]
                mask_offset = k_block_num - q_block_num + chunk_idx * num_blocks_per_chunk
...
def _sanitize_block_sparse_mask(
    ...
    if causal:
        # 允许 key block j <= query block i + offset
        offset = k_block_num - q_block_num
        causal_block = torch.tril(
            torch.ones(q_block_num, k_block_num, dtype=torch.bool, device=device),
            diagonal=offset,
        )
        mask = mask & causal_block[None, None, :, :]
    ...
    if keep_recent:
        ...
        for qi in range(q_block_num):
            kj = qi + offset
            if 0 <= kj < k_block_num:
                recent[qi, kj] = True
        mask = mask | recent[None, None, :, :]

    # 保底：每个 query block 至少保留一个合法 key block
    # 有些 block_sparse kernel 对全 False 行不安全。
    ...
    if empty.any():
        ...
        for qi in range(q_block_num):
            kj = qi + offset
            kj = max(0, min(kj, k_block_num - 1))
            mask[:, :, qi, kj] |= empty[:, :, qi]

    return mask.contiguous()


def _compute_average_block_density(
    mask: torch.Tensor,
    q_block_num: int,
    k_block_num: int,
    causal: bool = True,
    ...
    """
    Compute average sparse block density from the final block-sparse mask.

    Returns a Python float only when called. This intentionally calls .item(),
    so use return_density=True only for reporting/debugging, not inside timing.
    """
    mask = mask[:, :, :q_block_num, :k_block_num]
    batch_size, num_heads, _, _ = mask.shape

    selected_blocks = mask.sum(dtype=torch.float32)
    ...
    if causal:
        ...
        valid_blocks = causal_block.sum(dtype=torch.float32) * batch_size * num_heads
    else:
    ...
    density = selected_blocks / valid_blocks.clamp_min(1.0)
    return float(density.detach().cpu().item())
...
def _fixed_topk_mask_from_scores(
    ...
    """
    Build a hard mask by selecting the same per-query top-k budget from
    ...
    offset:
        Same offset previously passed to find_blocks_chunked:
            k_block_num - q_block_num + chunk_idx * num_blocks_per_chunk

    This replaces the threshold/top-p find_blocks_chunked selection only when
    fixed_topk is not None. Score computation and conv smoothing are unchanged.
    """
    ...
    # Fallback: avoid all-False rows when visible blocks are fewer than topk
    # or when numerical masking creates empty rows.
...
def Conv_prefill(
    ...
    if return_density:
        average_density = _compute_average_block_density(
            mask,
            q_block_num=q_block_num,
            k_block_num=k_block_num,
            causal=causal,
    ...
    del query_states, approx_simple_mask, attn_sums, mask
