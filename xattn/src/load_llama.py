...
try:
    ...
    print("Conv Prefill Import Fail")
...
def _maybe_unpack_density(result):
    """
    Prefill functions normally return attn_output.
    With return_density=True they should return (attn_output, density).
    This helper keeps the forward path robust if an older implementation is used.
    """
        ...
        if not decoding:
            if self.fastprefillconfig.metric == "flex":
                attn_output = Flexprefill_prefill(
                    ...
                    topk_ratio=self.fastprefillconfig.block_topk_ratio,
                ).transpose(1, 2)
            elif self.fastprefillconfig.metric == "xattn":
                if isinstance(self.fastprefillconfig.threshold, torch.Tensor):
                    threshold = self.fastprefillconfig.threshold[self.layer_idx].to(key_states.device)
                else:
                ...
                result = Xattention_prefill(
                    ...
                    use_triton=True,
                    topk_ratio=self.fastprefillconfig.block_topk_ratio,
                    return_density=self.fastprefillconfig.report_density,
                )
                attn_output, density = _maybe_unpack_density(result)
                if self.fastprefillconfig.report_density:
                    self.fastprefillconfig.add_density_record(
                        metric="xattn",
            ...
            elif self.fastprefillconfig.metric == "minference":
                attn_output = Minference_prefill(
                    query_states,
                    key_states,
                    value_states,
                )
            elif self.fastprefillconfig.metric == "conv":
                # Align Conv with XAttention: use the same per-layer threshold
                # and the same find_blocks_chunked selection path by default.
                if isinstance(self.fastprefillconfig.threshold, torch.Tensor):
                ...
                result = Conv_prefill(
                    ...
                    norm=1,
                    threshold=threshold,
                    use_triton=self.fastprefillconfig.conv_use_triton,
                    conv_weight_path=self.fastprefillconfig.conv_weight_path,
                    layer_idx=self.layer_idx,
                    conv_safe_topk=self.fastprefillconfig.conv_safe_topk,
                    fallback_topk=self.fastprefillconfig.conv_fallback_topk,
                    topk_ratio=self.fastprefillconfig.block_topk_ratio,
                    return_density=self.fastprefillconfig.report_density,
                )
                attn_output, density = _maybe_unpack_density(result)
...
class FastPrefillConfig(dict):
    ...
    def __init__(
        ...
        stride = 16,
        metric = "xattn",
        conv_weight_path: str = ".pt",
        conv_safe_topk: bool = False,
        conv_use_triton: bool = True,
        conv_fallback_topk: int = 8,
        # Shared sparse keep ratio used by xattn/conv/flex/minfer.
        ...
        # Conv-specific options. Default conv_safe_topk=False keeps Conv on
        # find_blocks_chunked, matching XAttention's mask-selection path.
        conv_weight_path = "/inspire/hdd/global_user/gexinmu-253108100065/Repos/fuyicheng_workshop/Innovator-lm-evaluation-hardness/x-attention-main/xattn/conv_weights/.pt"
        # print("conv_weight_path",conv_weight_path)
        self.conv_weight_path = conv_weight_path
        self.conv_safe_topk = conv_safe_topk
        self.conv_use_triton = conv_use_triton
        self.conv_fallback_topk = conv_fallback_topk
    ...
    def add_density_record(self, metric: str, layer_idx: int, density):
        ...
        if self.print_density_per_layer:
            print(
                f"[Density] metric={record['metric']} "
    ...
    def get_density_summary(self):
        """Return average density grouped by metric."""
        summary = {}
        for rec in self.density_records:
            metric = rec["metric"]
            density = rec["density"]
            if math.isnan(density):
                continue
            if metric not in summary:
                summary[metric] = {"sum": 0.0, "count": 0}
            summary[metric]["sum"] += density
            summary[metric]["count"] += 1

        return {
            metric: vals["sum"] / max(vals["count"], 1)
    ...
    def print_density_summary(self):
        ...
        if not summary:
            ...
            return
        msg = " ".join([f"{k}={v:.6f}" for k, v in summary.items()])
        print(f"[Density Summary] {msg}", flush=True)
