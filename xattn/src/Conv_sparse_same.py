...
# Global cache to avoid repeated disk I/O for conv weights.
_CONV_WEIGHT_CACHE = {}


# Default paths for conv kernel weights
_CONV_WEIGHT_DIR = os.path.join(os.path.dirname(__file__), "..", "conv_weights")
_DEFAULT_WEIGHT_PATH = "/inspire/hdd/global_user/gexinmu-253108100065/Repos/fuyicheng_workshop/Innovator-lm-evaluation-hardness/x-attention-main/xattn/conv_weights/conv_kernel_7x7_ruler_mix_guarded_t06_top16_1e4_step9500.pt"
...
def _choose_softmax_segment_size(k_reshaped_seq_len: int, reshaped_block_size: int) -> int:
    """
    Match the current XAttention.py behavior: use the minimal segment size,
    i.e. segment_size = reshaped_block_size.

    This disables the CONV_SOFTMAX_SEGMENT_SIZE=128 engineering optimization
    so Conv and XAttention use the same softmax_fuse_block_sum segment policy.
    """
    return int(reshaped_block_size)


# Static prefill tensor caching intentionally disabled for fair comparison
# with the current XAttention.py implementation.
