...
def flat_group_gemm_fuse_reshape_kernel(Q, K, Out, 
    ...
    # Keep the original 128x128 tile algorithm, but sample only one
    # residue class of the inverse anti-diagonal offsets.
    # Original: iter = 0, 1, ..., STRIDE-1.
    # New:      iter = phase, phase + SAMPLE_KERNEL, ...
    # phase is tile-dependent, so neighboring 128x128 tiles use different
    # sampled inverse diagonals while preserving large tensor-core tl.dot tiles.
    phase = (block_m + block_n) % SAMPLE_KERNEL

    q_mask_base = (tl.arange(0, BLOCK_M)[:, None] >= 0) & (tl.arange(0, HEAD_DIM)[None, :] >= 0)
    k_mask_base = (tl.arange(0, HEAD_DIM)[:, None] >= 0) & (tl.arange(0, BLOCK_N)[None, :] >= 0)

    for sample_id in range(NUM_SAMPLES):
        iter = phase + sample_id * SAMPLE_KERNEL
        valid = iter < STRIDE
        q = tl.load(
            Q_ptrs - iter * stride_qn,
            mask=valid & q_mask_base,
            other=0.0,
        )
        k = tl.load(
            K_ptrs + iter * stride_kn,
            mask=valid & k_mask_base,
            other=0.0,
        )
        o += tl.dot(q, k)
...
def flat_group_gemm_fuse_reshape(query_states, key_states, stride, chunk_start, chunk_end, is_causal=True, sample_kernel_size=7):
    batch_size, num_heads, q_len, head_dim = query_states.shape
    kv_len = key_states.shape[2]
    ...
    sample_kernel_size = max(1, min(int(sample_kernel_size), int(stride)))
    num_samples = int(math.ceil(float(stride) / float(sample_kernel_size)))
