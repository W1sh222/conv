...
# Conv 配置
# =========================
CONV_WEIGHT_PATH = "xattn/conv_weights/conv_kernel_7x7_ruler_like_t065_best.pt"
...
# Fixed top-k mode: set SAME_TOPK=True to benchmark XAttn and Conv under
# exactly the same per-query block budget. Threshold is still passed but ignored
# by Xattention/Conv when fixed_topk is not None.
SAME_TOPK = False
FIXED_TOPK = int(os.environ.get("FIXED_TOPK", "16"))
...
if __name__ == "__main__":
    ...
    for seq_k in lens:
        ...
        if SAME_TOPK:
            print(f"[same-topk mode] fixed_topk={FIXED_TOPK}")
            ...
            if XATTN_PREFILL:
                try:
                    Xattention_prefill(
                        ...
                        fixed_topk=FIXED_TOPK if SAME_TOPK else None,
                    )
        ...
        if MINFERENCE_PREFILL:
            avg_time_minfer = benchmark_cuda(
                lambda: Minference_prefill(k, q, v),
                num_iterations=num_iterations,
            )
        else:
