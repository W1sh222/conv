#!/usr/bin/env bash
# Shared Llama 128K RULER runner, based on run64_1/2/3.sh.
set -eo pipefail
...
source config_models_llama128.sh
MODEL_NAME=${1}
...
# Match the three existing 64K task partitions exactly.
case "${TASK_GROUP}" in
    1)
        source config_tasks3.sh
        ;;
    2)
        source config_tasks4.sh
        ;;
    3)
        source config_tasks5.sh
        ;;
    *)
        echo "Unsupported Llama 128K task group: ${TASK_GROUP}" >&2
        exit 2
        ;;
esac
