...
utility 越大，说明把该候选换入后正确答案 loss 越低。邻域特征改为两条方向线上的初始得分均值：候选中心所在的竖线，以及穿过中心的 `\\` 反斜线；每条线取中心两侧各 3 个块，因此每条线最多 6 个邻居、总计最多 12 个邻居。越界、因果非法位置和中心块自身都会被排除。这样不会把中心得分泄漏到邻域特征中。
