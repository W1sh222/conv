...
# 48K-64K T0.65 specialization for the original sparse Conv architecture.
# Start from the best short corrective T0.60 checkpoint, reset the optimizer,
# and emphasize the RULER multikey/QA2 cases where sparse inference still
# trails full attention. The inference-matched block size remains 128.
...
# plateaued T0.8 32k-40k checkpoints.
...
WEIGHT_DIR="${XATTN_ROOT}/conv_weights2"

DATA_SAMPLES="${DATA_SAMPLES:-16000}"
SYNTH_DATA="${SYNTH_DATA:-${NOLIMA_ROOT}/synth_train/ruler_mix_sparse_t065_48k64k_multikey_qa2_${DATA_SAMPLES}.jsonl}"

# This checkpoint was the best balance of hard recall and target coverage in
# the 24K-40K corrective run. Load weights only; do not restore its optimizer.
BEST_BASE_PATH="${BEST_BASE_PATH:-${WEIGHT_DIR}/conv_kernel_7x7_ruler_mix_sparse_guarded_t060_topkaware_24k40k_bf16_ema_step3250.pt}"
INIT_PATH="${INIT_PATH:-${BEST_BASE_PATH}}"
...
OUT_PATH="${OUT_PATH:-${WEIGHT_DIR}/conv_kernel_7x7_ruler_mix_sparse_guarded_t065_multikey_qa2_48k64k_${MODEL_PRECISION}.pt}"
LOG_DIR="./ft_scripts/sparse_ruler/logs"
mkdir -p "${LOG_DIR}" "${WEIGHT_DIR}" "$(dirname "${SYNTH_DATA}")"
LOG_FILE="${LOG_DIR}/train_sparse_guarded_t065_multikey_qa2_48k64k_${MODEL_PRECISION}.log"
...
# RESUME_STATE is only for recovering an interrupted run of this exact
# 48K-64K T0.65 experiment. Leave it empty for the intended fresh optimizer.
RESUME_ARGS=()
