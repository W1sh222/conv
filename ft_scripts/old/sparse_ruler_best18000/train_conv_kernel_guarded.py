...
class BoundedResidualConvKernel(nn.Module):
    """W = W0 + alpha * tanh(delta). Saves effective W."""

    def __init__(self, init_path: Optional[str], num_layers: int, num_heads: int, kernel_size: int, alpha: float):
        super().__init__()
        ...
        self.delta = nn.Parameter(torch.zeros_like(w0))
